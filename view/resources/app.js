...
function fillDeliveryAddressForEnterprise (select) {
	$('#order-input-enterprise-address-value').val($(select).find(":selected").val());
}

function fillDeliveryAddressForCustomer (select) {
	$('#order-input-customer-address-value').val($(select).find(":selected").val());
}
...
// update buttons
$("ul.cart-item").mouseover(function(e){
    $(this).children("div.update-area").css('display', 'block');
    e.preventDefault();
});
$("ul.cart-item").mouseleave(function(e){
    $(this).children("div.update-area").css('display', 'none');
    e.preventDefault();
});

$(".update-area").mouseleave(function(e){
    $(this).css('display', 'none');
    e.preventDefault();
});
...
$('#products-filter-form').find('input:checkbox').change(function(event) {
    showLoader();
    ...
    $.ajax({
        ...
        success: function(response) {
            if(response.code == "OK") {
                ...
                mapConfirmQuantity(popupConfirm, pntinput, pnt, response.object);
...
function singleProductWrapperAddToCart(pnt) {
    var pntinput = $('#'+pnt);
    var popupConfirm = $('.product-add-popup-confirm');
    popupConfirm.removeClass('is-visible');
    popupConfirm.children('.textarea').html('');
    ...
    $.ajax({
        ...
        success: function(response) {
            if(response.code == "OK") {
                mapCart(response.object);
                printSingleProductAddedItem(response.object, pnt);
                pntinput.val(1);
            } else if (response.code == "LOCKED") {
                mapSingleProductConfirmQuantity(popupConfirm, pntinput, pnt, response.object);
            } else if (response.code == "NOT_FOUND") {
                alert("Запрошенный товар недоступен");
            } else if(response.code == "INTERNAL_SERVER_ERROR") {
                serverAlert();
...
function updateCartItem(pnt) {
    var pntinput = $('#'+pnt);
    var popupConfirm = pntinput.parents('.cart-item').children('.product-edit-popup-confirm');
    popupConfirm.removeClass('is-visible');
    popupConfirm.children('.textarea').html('');
    ...
    $.ajax({
        ...
        dataType: "json",
        url: "/api/cart/update",
        data: addItemToJson(pnt, pntinput.val()),
        success: function(response) {
            if(response.code == "OK") {
                location.reload();
            } else if (response.code == "LOCKED") {
                hideLoader();
                mapEditConfirmQuantity(popupConfirm, pntinput, pnt, response.object);
            } else if (response.code == "NOT_FOUND") {
                hideLoader(); 
                alert("Запрошенный товар недоступен");
            } else if(response.code == "INTERNAL_SERVER_ERROR") {
...
function mapEditConfirmQuantity(popupConfirm, pntinput, pnt, quantityAvailable) {

    popupConfirm.children('.textarea').html('В наличии только: <strong style="color: red;">'+quantityAvailable[0]+'</strong> ед.');
    popupConfirm.children('a.confirm').attr('onclick', 'confirmEditToCart('+pnt+', '+quantityAvailable[0]+')');
    popupConfirm.addClass('is-visible');
}

function confirmEditToCart(pnt, quantity) {
    $('#'+pnt).val(quantity);
    updateCartItem(pnt);
}


function mapConfirmQuantity(popupConfirm, pntinput, pnt, quantityAvailable) {
...
function mapSingleProductConfirmQuantity(popupConfirm, pntinput, pnt, quantityAvailable) {

    popupConfirm.children('.textarea').html('В наличии только: <strong style="color: red;">'+quantityAvailable[0]+'</strong> ед.');
    popupConfirm.children('a.confirm').attr('onclick', 'confirmSingleProductAddToCart('+pnt+', '+(quantityAvailable[0] - quantityAvailable[1])+')');
    popupConfirm.addClass('is-visible');
}
...
function confirmSingleProductAddToCart(pnt, quantity) {
    $('#'+pnt).val(quantity);
    singleProductWrapperAddToCart(pnt);
}
...
function printSingleProductAddedItem(cart, pnt) {
    $('#in-cart-item').html('');
    $('#in-cart-item').html(
        '<div class="cart-is-added">'+
            '<div class="cart-is-added__pict">'+
...
function searchProducts() {
    ...
    delayTimer = setTimeout(function() {
        var searchStr = $('#search-popup-input').val();
        if(searchStr.replace(/\s/g, "").length > 0) {
