...
var cartFromHeader =    $('#popup-cart-from-header'),
    cartFromSearch =    $('#popup-cart-from-search');
...
//    product catalog stick to top
    ...
    $(window).scroll(function () {
        ...
        if (windowScroll > topOfObjToStick) { // Если прокрутили больше, чем расстояние до блока, то приклеиваем его
            ...
            if (cartFromHeader.css('display') != 'none') {
                cartFromHeader.hide();

                displayCart(cartFromSearch);
            ...
            if (cartFromSearch.css('display') != 'none') {
                cartFromSearch.hide();

                displayCart(cartFromHeader);
    ...
    /*var objToStick1 = $('.orderBlock-right'); //Получаем нужный объект
    if(objToStick1) {
        ...
        $(window).scroll(function () {
            ...
            if (windowScroll > topOfObjToStick1) { // Если прокрутили больше, чем расстояние до блока, то приклеиваем его
             $('.orderBlock-right').addClass('orderfix');
             } else {
               $('.orderBlock-right').removeClass('orderfix');
           }
        });
    }*/

    if (document.documentElement.clientWidth > 1200) {
        $('.order-menu-bar').theiaStickySidebar({
            // setting
...
//mainPageCart view control =======================================================================
var  cart=$('.Mydropdown-menu.menu-cart');
function cartDisplayTrigger() {

    var cartElements=cart.find('.cart-cont > li'); 
    var cartCloseButton = $('.closeCart');

    if ($('.fixed-basket').css('display') != 'none') {
        if (cartFromSearch.css('display') == 'none') {

            if (cartFromHeader.css('display') != 'none') cartFromHeader.hide();
            ...
            if(window.innerWidth>=992) {
                ...
                cartFromSearch.css({'display':'block','width' : window.innerWidth+'px', 'height': cartHeight+'px', 'right':0, 'bottom': bottom+'px'});
                cartCloseButton.removeAttr('style');
            }
        } else {
            if(window.innerWidth>=992){

                // cartCloseButton.css('display','none');
                for (var i = cartElements.length-1; i >=0 ; i--) {
                    $(cartElements[i]).delay((cartElements.length-i)*100).animate({
                        'left':'100%'
                    },300);
                }
                cartFromSearch.delay(cartElements.length*100).animate({
                    'width': '0',
                    'height': '0'
                },500, function(){cartFromSearch.css('display','none')});
            } else{
                // cartCloseButton.css('display','none');
                cartFromSearch.css('display','none');
            }
        }
    } else {
        ...
        if (cartFromHeader.css('display') == 'none') {
            ...
            if (cartFromSearch.css('display') != 'none') cartFromSearch.hide();
            ...
            if(window.innerWidth>=992) {
                ...
                cartElements.css({'left':'0'});
                cartFromHeader.css({'display':'block','width' : window.innerWidth+'px', 'height': cartHeight+'px', 'right':0, 'bottom': bottom+'px'});
                cartCloseButton.removeAttr('style');
            }
        } else {
            if(window.innerWidth>=992){
                ...
                cartFromHeader.delay(cartElements.length*100).animate({
                    'width': '0',
                    'height': '0'
                },500, function(){cartFromHeader.css('display','none')});
            } else{
                // cartCloseButton.css('display','none');
                cartFromHeader.css('display','none');
            }
        }
...
function displayCart(cart) {
    ...
    for (var i = 0; i < cartElements.length; i++) {
        $(cartElements[i]).delay(200+i*100).animate({
            'left':'0'
        },500, function(){cartCloseButton.removeAttr('style')});
    }
}

var cartControl=$('.dropdown.cart a.dropdown-toggle, .closeCart, .popup-close-cart');
cartControl.click(function(){
    cartDisplayTrigger();
});
//END mainPageCart view control =======================================================================
